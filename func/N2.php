...
add_action( 'init', array('N2', 'init') );

class N2 {
	function init() {
	    load_theme_textdomain( 'n2', get_template_directory() . '/languages' );
		add_filter( 'the_content', 'make_clickable' );
		if ( isset( $_REQUEST['n2ajax'] ) ) {
		...
		$is_media_upload = isset( $_REQUEST['n2-upload'] );
		...
		if ( $is_media_upload ) {
			...
			add_filter( 'admin_url', array('N2', 'url_filter') );
			add_filter( 'includes_url', array('N2', 'url_filter') );
			add_filter( 'script_loader_src', array('N2', 'url_filter') );
			add_filter( 'wp_get_attachment_url', lambda( '$url', 'str_replace(get_bloginfo("url")."/", site_url("/"), $url);'), 11);
			add_filter( 'media_upload_form_url', lambda( '$url', 'add_query_arg( array( "n2-upload" => "true" ), $url );' ) );
		}
	}
	function media_buttons2() {
		...
		return N2::make_media_urls( ob_get_clean() );
	}
	...
	function admin_url( $path ) { return N2::url_filter( admin_url( $path ) ); }
